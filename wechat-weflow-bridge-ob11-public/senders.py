...
def create_sender() -> BaseSender:
    ...
    重要：WeFlow 的 HTTP API 完全只读（仅 /api/v1/messages、/sessions、
    /contacts、SSE 推送等 GET 接口，全局不处理 POST），不存在发送消息的
    接口，因此 weflow_api 模式无法工作。若误选，自动回退为 uia（UI 自动化发送），
    避免踩坑。
    """
    if SEND_METHOD == "weflow_api":
        log.warning("⚠️ weflow_api 发送模式不可用（WeFlow HTTP API 只读、无发送接口），已自动回退为 UIA 发送")
        return UiaSender()
    else:
