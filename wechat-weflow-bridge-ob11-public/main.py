...
from bridge_core import WeFlowBridge, _refresh_contact_map
...
def _bridge_loop():
    ...
    # 预加载 微信ID→显示名 映射（群消息 SSE 只给 chatroom ID，需据此解析真实群名）
    _refresh_contact_map(force=True)

    # 每 5 分钟兜底刷新映射，捕捉新加的群/联系人
    def _periodic_contact_refresh():
        while state.running:
            time.sleep(300)
            if not state.running:
                break
            _refresh_contact_map()

    threading.Thread(target=_periodic_contact_refresh, daemon=True, name="contact-refresh").start()
