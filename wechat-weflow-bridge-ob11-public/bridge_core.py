...
# ============ 微信 ID → 显示名 映射 ============
# 群消息的 SSE 推送里 groupName 经常为空，sourceName/sessionId 给的是
# 54664630460@chatroom 这种内部 ID，拿这个去微信搜索框切群永远切不到。
# 必须从 WeFlow /api/v1/contacts 拉 username→displayName（群名/联系人名）来解析。


def _refresh_contact_map(force: bool = False):
    """从 WeFlow /api/v1/contacts 拉取 username→displayName 映射。"""
    now = time.time()
    # 节流：非强制刷新且 120s 内已更新过则跳过，避免消息洪峰下狂打网络
    if not force and (now - state._contact_map_updated) < 120:
        return
    try:
        url = f"{config.WE_FLOW_BASE_URL}/api/v1/contacts"
        r = requests.get(url, params={"access_token": config.ACCESS_TOKEN}, timeout=10)
        if r.status_code != 200:
            log.warning(f"[contacts] 加载失败 HTTP {r.status_code}")
            return
        j = r.json()
        contacts = j.get("contacts") or []
        m = {}
        for c in contacts:
            uname = c.get("username") or c.get("wxid") or ""
            disp = c.get("displayName") or c.get("nickname") or uname
            if uname:
                m[uname] = disp
        with state._contact_map_lock:
            state.contact_name_map = m
            state._contact_map_updated = now
        log.info(f"[contacts] 已加载 {len(m)} 个会话名称映射")
    except Exception as e:
        log.warning(f"[contacts] 加载异常: {e}")


def _resolve_contact_name(username: str) -> str:
    """根据微信 username/chatroom ID 解析显示名；映射缺失时按需刷新一次。"""
    if not username:
        return ""
    with state._contact_map_lock:
        name = state.contact_name_map.get(username)
        if name:
            return name
    # 缺失：强制刷新一次再试（新群/刚启动等情况）
    _refresh_contact_map(force=True)
    with state._contact_map_lock:
        return state.contact_name_map.get(username, "")
...
class WeFlowBridge:
    ...
    def add_to_buffer(self, data):
        ...
        if content == "[图片]":
            if not config.REPLY_TO_IMAGES:
                log.info("⏭️ 已禁用图片回复，跳过图片消息")
                return
        ...
        if is_group:
            ...
            # SSE 经常只给 chatroom ID（如 54664630460@chatroom），拿这个去微信搜索框
            # 切群永远切不到 → 退回坐标点击 → 发到当前窗口。这里用 WeFlow 联系人映射
            # 把 ID 解析成真实群名（如"测试"），后续 _switch_contact 才能正确切群。
            if (not base_name) or "@chatroom" in base_name:
                resolved = _resolve_contact_name(session_id_data) or _resolve_contact_name(source_name) or base_name
                if resolved:
                    base_name = resolved
            contact = base_name
        else:
