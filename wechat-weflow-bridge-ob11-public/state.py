...
# 微信 username(聊天ID/chatroom ID) → 显示名 映射，来源 WeFlow /api/v1/contacts。
# 群消息的 SSE 推送里 groupName 经常为空、sourceName 给的是 54664630460@chatroom 这种
# 内部 ID，无法直接拿去微信搜索框切群；必须靠这个映射把 ID 解析成真实群名。
contact_name_map: dict[str, str] = {}
_contact_map_lock = threading.Lock()
_contact_map_updated = 0.0
